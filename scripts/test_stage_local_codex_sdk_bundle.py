...
class StageLocalCodexSdkBundleTest(unittest.TestCase):
    ...
    def test_main_preserves_debug_target_by_default_before_building_codex(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_dir = Path(tmp)
            output_dir = tmp_dir / "npm-local"
            codex_bin = tmp_dir / "codex.exe"
    ...
    def test_main_can_clean_debug_target_before_building_codex(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_dir = Path(tmp)
            output_dir = tmp_dir / "npm-local"
            codex_bin = tmp_dir / "codex.exe"
