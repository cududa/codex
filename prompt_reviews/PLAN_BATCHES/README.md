...
| 5 | `04_services_workflow_state_machine.md` | In progress | Split into milestones to avoid a monolithic service implementation. Milestone A accepted service context/errors, status recomputation, review queue/remaining-work queries, and narrow repository helpers. Verified locally: `npm run typecheck`, `npm run test:structure`, `npm run test` (20 files / 76 tests), `npm run db:check`, and `npm run build`. Next milestone: classification/comment services on top of status refresh. Status override remains a known schema/command gap; no fake persistence added. |
