...
| 5 | `04_services_workflow_state_machine.md` | In progress | Split into milestones to avoid a monolithic service implementation. Milestone A accepted service context/errors, status recomputation, review queue/remaining-work queries, and narrow repository helpers. Milestone B accepted transaction-wrapped classification/comment services with scope/anchor invariants and status refresh. Verified locally: `npm run typecheck`, `npm run test:structure`, `npm run test` (22 files / 84 tests), `npm run db:check`, and `npm run build`. Next milestone: decision/plan/version services. Status override and classification summary/risk/confidence remain known schema/command gaps; no fake persistence added. |
