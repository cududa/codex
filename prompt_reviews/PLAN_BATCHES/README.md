...
| 5 | `04_services_workflow_state_machine.md` | In progress | Split into milestones to avoid a monolithic service implementation. Milestone A accepted service context/errors, status recomputation, review queue/remaining-work queries, and narrow repository helpers. Milestone B accepted transaction-wrapped classification/comment services with scope/anchor invariants and status refresh. Milestone C accepted decision/plan/version services with supported plan links: plan-comment, plan-decision, and plan-item commit-file/decision links. Verified locally: `npm run typecheck`, `npm run test:structure`, `npm run test` (25 files / 98 tests), `npm run db:check`, `npm run build`, `npm run schema:json`, and `npm run db:generate`. Direct plan-to-diff-block links remain Milestone D because current domain plan scope excludes diff blocks and persistence has no `plan_diff_blocks` table; do not fake this with raw IDs. Status override and classification summary/risk/confidence remain known schema/command gaps; no fake persistence added. |
