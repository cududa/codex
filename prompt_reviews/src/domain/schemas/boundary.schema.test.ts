...
const concernMapEntry = {
  slug: "harness-prompts",
  label: "Harness Prompts",
  behaviorDefinition: "Model-facing instruction sources and request construction.",
  seedPaths: [concernSeedPath],
  seedGlobPatterns: ["codex-rs/core/templates/model_instructions/*.md"],
  seedSymbols: ["build_responses_request"],
  seedStringMarkers: ["base_instructions"],
  seedTemplateMarkers: ["{{ personality }}"],
  expansionEdgeTypes: ["calls"],
  falsePositiveExclusions: ["Exclude unrelated TUI copy."],
  fixtureExpectations: [{ name: "fixture", description: "Expected detector fixture.", required: true }],
};
const concernGraphNode = {
  id: "cgn-1",
...
const concernGraphEdge = {
  id: "cge-1",
  concernSlug: "harness-prompts",
  edgeKey: "harness-prompts:calls:client",
  edgeKind: "calls",
  fromNodeId: "cgn-1",
  toNodeId: "cgn-2",
...
const detectorFinding = {
  ...
  findingKey: "commit-1:cf-1:harness-prompts",
  concernSlug: "harness-prompts",
  target: fileScope,
  path: "codex-rs/core/src/client.rs",
  side: "new",
...
describe("boundary view and response schemas", () => {
  it("parses representative view payloads", () => {
    const responses = [
      ...
      [ConcernSeedPathSchema, concernSeedPath],
      [ConcernMapEntrySchema, concernMapEntry],
      [ConcernGraphNodeSchema, concernGraphNode],
      [ConcernGraphEdgeSchema, concernGraphEdge],
      [DetectorRunSchema, detectorRun],
      [DetectorFindingSchema, detectorFinding],
      [DetectorFindingSummarySchema, detectorFindingSummary],
