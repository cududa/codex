...
export class ReviewStoreError extends Error {
  constructor(
    readonly code: Extract<ApiErrorCode, "bad_request" | "not_found" | "state_conflict">,
    message: string,
  ) {
    super(message);
...
export function badRequest(message: string): ReviewStoreError {
  return new ReviewStoreError("bad_request", message);
}
